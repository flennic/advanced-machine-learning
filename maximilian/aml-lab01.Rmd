...
First we will extract the *Markov Blanket* using the function `mb()`. The *Markov Blanket* are all nodes that shield the node in question from the rest of the network. Then we apply the same predecure as before and look at the results.

```{r}

markov_blanket = mb(bn_1, "S")

# predict for each data point of the test data
res_mb = t(apply(testX, 1, FUN = function(x) {
  return(querygrain(setEvidence(bn_1_grain, markov_blanket, x[markov_blanket]))$S)
}))

# classify
pred_mb = apply(res_mb, 1, FUN = function(x) {
  if (x[2] > 0.5) return("yes")
  return("no")
})

# Factorize
pred_factor_mb = factor(pred_mb)

# Call to a library to calculate interesting metrics
confusionMatrix(pred_factor_mb, testY_factor, mode="everything")
...
The Naive Bayes classifier assumes independence between all the predictive variables. Therefore the classifier is given by
...
As the can see, according to our model there exist no dependence between the features. Let's train the parameters and look at the results.

```{r}

bn_naive_bayes_fit = bn.fit(bn_naive_bayes, train)
bn_naive_bayes_grain = as.grain(bn_naive_bayes_fit)

# predict for each data point of the test data
res_naive_bayes = t(apply(testX, 1, FUN = function(x) {
  return(querygrain(setEvidence(bn_naive_bayes_grain, names(x), x))$S)
}))

# classify
pred_naive_bayes = apply(res_naive_bayes, 1, FUN = function(x) {
  if (x[2] > 0.5) return("yes")
  return("no")
})

# Factorize
pred_factor_naive_bayes = factor(pred_naive_bayes)

# Call to a library to calculate interesting metrics
confusionMatrix(pred_factor_naive_bayes, testY_factor, mode="everything")
