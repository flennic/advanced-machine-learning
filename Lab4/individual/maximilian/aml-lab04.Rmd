...
# Parameters
sigma_n = sd(lm(temp ~ time, data = temp_stockholm_reduced)$res)
sigma_f = 20
ell = 0.2
...
# Covariance
Covf = gp_covariance(temp_stockholm_reduced$temp,
                     temp_stockholm_reduced$temp,
                     se_kernel,
                     sigma_n)
...
sigma_n = sd(lm(temp ~ day, data = temp_stockholm_reduced)$res)
sigma_f = 20
ell = 0.2
...
#' @param ell_2 Ell_2.
#'
#' @return Returns fresh functions directly from the canary island! Sweet!
periodic_kernel_factory = function(sigma_f, d, ell_1, ell_2) {
  kf = function(x, y) {
    return(sigma_f^2 *
             exp(2 * sin(pi * abs(x - y)/d)/ell_1^2) *
