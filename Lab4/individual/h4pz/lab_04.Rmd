...
plot_me_pls = function(model, X_star, X, y, confidence=0.95, title='GP Posterior')
{
...
`kernlab` can compute the posterior variance of $f$, but it seems to be a bug in the code. So, do your own computations for the posterior variance of $f$, but it seems to be a bug in the code. So, do your own computations for the posterior variance of $f$ and plot the $95\%$ probability (pointwise) bands for $f$. Superimpose thse bands on the figure with the posterior mean that you obtained in (2).

**Hint**: Note that Algorithm 2.1 on page 19 of Rasmussen and Willams' book already does the calculations required. Note also that `kernlab` scaled the data by default to have ero mean and standard deviation one. So, the output of your implementation of Algorithm 2.1 will not coincide with the ouput of `kernlab` unless you scale the data first. For this, you may want to use the `R` function `scale`.
...
get_gp_variance = function(X, y, X_star, sigma_noise, kernel)
{
  ...
  # Calculating the predictive variance.
  var_f_star = K_test - t(K_star) %*% solve(K + diag(sigma_noise, n)) %*% K_star
...
For the `time` model, the pros are that is a model that is simpler compared to the `time` model. However it's a model that doesn't capture seasonality since it's basically learning functions over the indexes of the observations. In the other hand, the `day` model is able to capture these seasonality patterns. Unfortunately, the `day` model can suffer from overfitting and thus requires some thinkering with the kernel parameters in order to introduce some smoothness over the function space.
...
**Answer**
From the plots and the results we can see that the model with a preiodic kernel is some what an intermediate step between the `day` model and the `time` model. Sometimes it behaves more closely to $f(day)$ and sometimes to $f(time)$. This is also reflected on it's MSE since it is in between the other two models.
